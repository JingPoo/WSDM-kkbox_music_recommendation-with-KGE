...
for emb_size in [20,15,10]:
    for neg_ratio in [16,8,4]:
        ...
        rum_name = 'lr0001-emb' + str(emb_size) + 'neg' + str(neg_ratio)
        log_path = './tensorboard_logs/lr0001-emb' + str(emb_size) + 'neg' + str(neg_ratio)

        with mlflow.start_run(run_name = rum_name):
            ...
            # train the model
            model.train(train_X=train, val_X=valid, metadata=metadata, epochs=100, batch_size=10000,
            ...
            # free memory
            del train, valid, test, metadata, test_data
            gc.collect()
