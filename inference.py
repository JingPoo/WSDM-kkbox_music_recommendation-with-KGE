...
# (1) users: 30% inference, 70% remain
distinct_users = [entity for entity in type_dict.keys() if type_dict[entity] == 'member']
remain, inference = train_test_split(distinct_users, test_size=0.3)
# (2)
...
# conver train_remain into index
...
# (4) only use train_remain to train nodel
...
run_name = 'inference30-emb' + str(emb_size) + 'neg' + str(neg_ratio)
log_path = './tensorboard_logs/inference30-emb' + str(emb_size) + 'neg' + str(neg_ratio)
...
with mlflow.start_run(run_name = run_name):
        ...
        # train the model
        model.train(train_X=train_remain, val_X=valid, metadata=metadata, epochs=2, batch_size=10000,
                early_stopping_rounds=10, restore_best_weight=False,
                optimizer=tf.optimizers.Adam(learning_rate=0.0001),
                seed=12345, log_path=log_path, log_projector=True)
        ...
        plt.ylabel('Loss')
        plt.legend()
        fig_fn = 'inference30_train_val_loss_{}_{}.png'.format('emb' + str(emb_size),'neg' + str(neg_ratio))
        plt.savefig(fig_fn)
