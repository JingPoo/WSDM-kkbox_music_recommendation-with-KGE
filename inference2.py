...
def inference_recommend(user_list):
    ...
    for users in tqdm.tqdm(batch(user_list,100), total=len(user_list)//100+1):
        ...
        inference_users_emb = tf.convert_to_tensor([user_and_inferenceEmb[user].numpy() for user in users])
        ...
        compute_songs_emb = inference_users_emb + has_interest_emb
...
def remain_recommend(user_list):
    ...
    for users in tqdm.tqdm(batch(user_list,100), total=len(user_list)//100+1):
        ...
        # compute recommend songs (batch_users * embedding_size)
        compute_songs_emb = remain_users_emb + has_interest_emb
